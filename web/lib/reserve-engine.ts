...
async function post(
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    ...
    if (attempt > 0) {
      ...
      addLog(taskId, 'warn', `${path} 第 ${attempt} 次重试（等待 ${delay}ms）...`);
    ...
    if (data?.status === 'error') throw new Error(`业务错误: ${data.message ?? '未知'} on ${path}`);
    return data;
  }
  throw lastError!;
}
...
async function syncServerTime(token: string, signal: AbortSignal, taskId: string) {
  ...
  addLog(taskId, 'info', `服务器时间同步完成，offset: ${offset >= 0 ? '+' : ''}${offset}ms`);
  return offset;
}
...
async function waitUntilOpen(offset: number, signal: AbortSignal, taskId: string) {
...
async function tryTimeSlot(
  ...
  function buildOrderData(site_id: number) {
    return { openid, nickname, phone, date, venue_name: VENUE_NAME, venue_type: VENUE_TYPE,
      site_id, total_price: 0, time_list, start_time, end_time };
  }
    ...
    for (const site_id of PREFERRED_SITES) {
      ...
      post('/creat_book_info', token, { orderData }, signal, taskId)
        .then(async (checkRes) => {
          ...
          try {
            const orderRes = await post('/creat_order', token, { orderData }, signal, taskId);
        ...
        .catch((e: Error) => {
          if (!signal.aborted) addLog(taskId, 'error', `  场地 ${site_id} 查询异常: ${e.message}`);
        })
        .finally(() => { pending--; checkPending(); });
    }
  });
}
...
export async function executeReserve(taskId: string) {
  ...
  const { token, date, openid, nickname, phone, preferred_time_slots, abortController } = task;
  ...
  for (let i = 0; i < preferred_time_slots.length; i++) {
    ...
    const result = await tryTimeSlot(token, { date, openid, nickname, phone }, slot, signal, taskId);

    if (result) {
      const r = result as {
        order_num: string; venue_name: string; site_id: number;
        order_date: string; start_time: string; end_time: string;
      };
      addLog(taskId, 'success', `预约成功！订单号: ${r.order_num}`);
      addLog(taskId, 'success',
